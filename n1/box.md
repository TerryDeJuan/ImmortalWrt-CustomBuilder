...
#### Web interface address for single-port models: `None; must be looked up`. Find the device's LAN IP address in the DHCP client list of the upstream router.
#### Web interface address for multi-port models: `192.168.100.1`
...
#### The img file is packaged with `ophub/amlogic-s9xxx-openwrt@main`.
#### Default base-image location: https://github.com/wukongdaily/AutoBuildImmortalWrt/releases/tag/rootfs

#### The Releases in this project are only for the author's testing and are deleted periodically. Fork the project and build the firmware yourself with Actions.

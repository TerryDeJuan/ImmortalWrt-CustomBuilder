...
# A limited number of third-party APK packages are currently supported; enable them by uncommenting entries in shell/apk-custom-packages.sh
...
if [ -z "$CUSTOM_PACKAGES" ]; then
  ...
  # Copy all run and apk files under run/x86 to the extra-packages directory
...
# Merge third-party plugins from outside the ImmortalWrt repository; temporarily commented out
